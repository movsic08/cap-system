<div x-data="{createOpen:false}" class="inline">
    <button x-on:click="createOpen = true"
        ...
        <span class="xs:block text-sm ml-2">Add Images</span>
    </button>
    ...
    <div x-show="createOpen" x-cloak>
        <div class="absolute bg-slate-50 shadow-md rounded-md top-2/4 left-2/4 w-3/4 -translate-y-2/4 -translate-x-2/4"
            ...
            <header class="flex justify-between items-center  p-4 border rounded-md border-b-1 border-gray-200">
                <h2 class="font-medium">Gallery Images</h2>
            ...
            <form class=" p-4" action="{{ route('gallery.store') }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div>
                    <x-input-label class="text-slate-800" for="images" :value="__('Images')" />
                    <x-text-input id="images" class="block mt-1 w-full" type="file" name="images[]" multiple
                        accept="image/*" :value="old('images')" required autofocus />
