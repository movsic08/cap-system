<div x-data="{createOpen:false}" class="inline">
    <button x-on:click="createOpen = true"
        ...
        <span class="xs:block text-sm ml-2">Create Mass</span>
    </button>
    ...
    <div x-show="createOpen" x-cloak>
        <div class="absolute bg-slate-50 shadow-md rounded-md top-2/4 left-2/4 w-3/4 -translate-y-2/4 -translate-x-2/4"
            ...
            <header class="flex justify-between items-center  p-4 border rounded-md border-b-1 border-gray-200">
                <h2 class="font-medium">Add Mass</h2>
            ...
            <form class="p-4" action="{{ route('offertory.store') }}" method="POST">
                @csrf
                <div>
                    <x-input-label class="text-slate-800" for="mass" :value="__('Mass')" />
                    <x-text-input id="mass" class="block mt-1 w-full" type="text" name="mass" :value="old('mass')"
                        required autofocus />
                    <x-input-error class="mt-2" :messages="$errors->get('mass')" />
                </div>

                <div class="mt-4">
                    <x-input-label class="text-slate-800" for="date" :value="__('Date')" />
                ...
                <div class="mt-4">
                    <x-input-label class="text-slate-800" for="details" :value="__('Details')" />
